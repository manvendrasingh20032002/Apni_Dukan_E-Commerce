            ...
            <div className="container-fluid my-3">
                <div className="row">
                    ...
                    <div className="col-md-9">
                        <h6 className='mybackground text-light text-center p-2 fs-1 mb-3'>Product
                            <Link to="/admin/product/create"><i className='bi bi-plus text-light fs-1 float-end'></i></Link>
                        </h6>
                                ...
                                <tbody>
                                    {data.map(item => {
                                        return <tr key={item._id}>
                                                ...
                                                <div style={{width:500}}>
                                                    {
                                                        item?.pic?.map((p, index) => {
                                                            return <Link key={index} to={`${import.meta.env.VITE_APP_IMAGE_SERVER}${p}`} target='_blank'>
                                                                <img src={`${import.meta.env.VITE_APP_IMAGE_SERVER}${p}`} className='m-1' height={50} width={50} alt="" />
                                                            </Link>
                                                        })
                                                    }
                                                </div>
                                            ...
                                            <td><Link to={`/admin/product/update/${item._id}`} className='btn btn-primary mybackground'><i className='bi bi-pencil fs-3'></i></Link></td>
